...
from custom_model import BasicCNN, IntermediateCNN, AdvancedCNN, OptimizedCNN
from custom_model_demo import CustomCNNMinus, CustomCNN, CustomCNNPlus, CustomCNNPro
...
def main():
    ...
    FINAL_MODEL_PATH = "models/CustomModel/Pro/final_model_pro.pth"
    TENSOR_BOARD_PATH = "runs/CustomModel/Pro"
    ...
    shutil.rmtree(TENSOR_BOARD_PATH, ignore_errors=True)  # Xóa thư mục runs nếu tồn tại
    writer = SummaryWriter(TENSOR_BOARD_PATH)  # Ghi log vào thư mục cố định
    ...
    train_dataset = CustomDataset(
        TRAIN_PATH, transform=train_transform, max_samples=None
    )
    val_dataset = CustomDataset(
        VAL_PATH, transform=val_test_transform, max_samples=None
    )
    test_dataset = CustomDataset(
        TEST_PATH, transform=val_test_transform, max_samples=None
    )
    ...
    # Sử dụng model
    # model = CustomCNNMinus(num_classes)
    # model = CustomCNN(num_classes)
    # model = CustomCNNPlus(num_classes)
    model = CustomCNNPro(num_classes)

    # model = BasicCNN(num_classes)
    # model = IntermediateCNN(num_classes)
    # model = AdvancedCNN(num_classes, width_mult=1.0)
    # model = OptimizedCNN(num_classes, width_mult=1.0, depth_mult=1.0)
